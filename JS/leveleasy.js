...
const blocks = [
   { id: 'block1', x: 500, y: 250, width: 50, height: 50 },
   { id: 'block2', x: 800, y: 350, width: 50, height: 50 },
   { id: 'block3', x: 300, y: 450, width: 50, height: 50 },
   { id: 'block4', x: 550, y: 550, width: 50, height: 50 },
];
...
function updateBotPosition() {
   ...
   tools.forEach(tool => {
      ...
      if (!tool.path || tool.pathIndex === undefined) {
         ...
         tool.pathIndex = 2; // pocatecni bod
         tool.speed = 3; // rychlost bota
      }
            ...
            if (gameRunning) {
               ...
               for (const block of blocks) {
                  ...
                  if (isColliding(bulletRect, blockRect)) {
                     ...
                     // missions
                     if (!paintBlocks.includes(block)) {
                        console.log("+ 1 coin for paint block");
                        switch (block.id) {
                           case "block1":
                              mis1.innerHTML = "star";
                           ...
                           case "block4":
                              ...
                              break;
                           default:
                              console.log("null blcok");
                        }
                        coinsV += 1;
                        paintBlocks.push(block);
                        coins.innerHTML = coinsV;
                     }
                     if (paintBlocks.length == 4) {
                        missw = true;
                     }
                     paintBlocks.sort();
